...
export async function createPromotionActionWithState(
  _prevState: { createdAt: number; promotionId: string; error?: string },
  formData: FormData
) {
  try {
    const title = parseRequiredText(formData.get("title"));
    ...
    return {
      createdAt: Date.now(),
      promotionId: "",
      error: (error as Error).message || "No se pudo crear la promoción.",
    };
  }
}
