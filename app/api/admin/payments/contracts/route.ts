...
export async function GET(request: Request) {
  ...
  const sortedRows = sortContractsByFolioDesc(contractIndexRows);
  const contractIds = sortedRows
    .slice(offset, offset + CONTRACT_LIMIT)
  ...
  return NextResponse.json({
    contracts,
    loadedCount: Math.min(offset + contracts.length, sortedRows.length),
